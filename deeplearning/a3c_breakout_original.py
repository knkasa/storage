...
class A3C:
    def __init__(self, id_name, master_model, env, epsilon, im_size, m, batch_size=64):
        ...
        self.env = env
        self.input_size = self.env.observation_space.shape[0]         # image will be truncated, so this will not be used.  
        self.action_size = self.env.action_space.n       # number of action=4
    ...
    def local_network(self):
        ...
        self.hidden_layer1 = tf.keras.layers.Conv2D( 32, 8, (4,4), activation='relu', padding='same',  name='hidden_layer1' )(self.input_layer) 
        self.hidden_layer2 = tf.keras.layers.Conv2D( 64, 4, (2,2), activation='relu', padding='valid', name='hidden_layer2' )(self.hidden_layer1) 
        self.hidden_layer3 = tf.keras.layers.Conv2D( 64, 3, (1,1), activation='relu', padding='valid',  name='hidden_layer3' )(self.hidden_layer2) 
        self.flat_layer = tf.keras.layers.Flatten(name='flat_layer')(self.hidden_layer3)
        self.dense_layer = tf.keras.layers.Dense( 512, activation='elu',  name='dense_layer' )(self.flat_layer)
        self.policy_layer = tf.keras.layers.Dense( self.action_size, activation='softmax',  name='policy_layer' )(self.dense_layer)  
        self.value_layer = tf.keras.layers.Dense( 1, name='value_layer' )(self.dense_layer)
    ...
    def train(self, tape, reward_list, policy_list, value_list, action_list, batch_size):    # this is for training.    
        ...
        loss = 1.0*value_loss + policy_loss + 0.01*entropy
    ...
    def play(self):
        ...
        for e in range(EPOCHS):
            ...
            with tf.GradientTape(persistent=True, watch_accessed_variables=False) as tape:
                ...
                if self.id==0:
                    print( " ID = ", self.id, " Episode = ", e,  " Score = ", sum_reward,
...
class Worker:
    def __init__(self, id_name, global_counter, master_model, env, epsilon, im_size, m ):
        self.id = id_name
        ...
        self.agent = A3C( self.id, self.master_model, self.env, self.epsilon, self.im_size, self.m )
...
class lancher:
    def __init__(self):
        ...
        with tf.device("/cpu:0"):
            ...
            input_size = env.observation_space.shape[0]       # raw image size (note that image will be truncated)
            self.action_size = env.action_space.n       # number of actions = 4
            ...
            for worker_id in range(NUM_WORKERS):
                worker = Worker(worker_id, global_counter, self.master_model, env, epsilon, self.im_size, self.m )
    ...
    def master_network(self):
        ...
        self.hidden_layer3 = tf.keras.layers.Conv2D( 64, 3, (1,1), activation='relu', padding='valid', name='hidden_layer3' )(self.hidden_layer2) 
        self.flat_layer = tf.keras.layers.Flatten(name='flat_layer')(self.hidden_layer3)
        self.dense_layer = tf.keras.layers.Dense( 512, activation='elu' )(self.flat_layer)
        self.policy_layer = tf.keras.layers.Dense( self.action_size, activation='softmax', name='policy_layer' )(self.dense_layer)  
        self.value_layer = tf.keras.layers.Dense( 1, name='value_layer' )(self.dense_layer)
