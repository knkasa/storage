...
time = np.linspace( 0, 15*np.pi, 201 ) 
out = np.sin( time )
...
X = np.array(X).reshape(-1, T, D)    # make it NxTxD.  N=# of rows (samples) to train
Y = np.array(Y)
N = len(X)
...
res = model.fit( X[:-N//2], Y[:-N//2], batch_size=32, epochs=200, validation_data=( X[-N//2:], Y[-N//2:])  )
#res = model.fit( X[:-N//2], Y[:-N//2], batch_size=32,  validation_data=( X[-N//2:], Y[-N//2:])  )
...
plt.plot( predicted[:], 'o-', label='forecast' )
plt.legend()
plt.show()
