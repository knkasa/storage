...
class DQN:
    def __init__(self, env_string, batch_size=64):
        ...
        self.epsilon = 0.1        # used for epsilon-greedy actions.  1.0 means always take random action.
        ...
        for e in range(EPOCHS):
            ...
            if mean_score >= THRESHOLD:
                ...
                print( e, " episodes run. Survival time = ", i, ".  mean_score(last 20 trials) = ", 
                        np.round( mean_score, 2), " epsilon = ", self.epsilon )
                
            self.replay( self.batch_size )  # train one episode.
            #self.epsilon = 0.99*self.epsilon   # Adjust epsilon every episode.  
...
for n in range(20):
    ...
    sum_reward = 0
    while not done:
        agent.env.render()   # agent.env.env.render()
        state = agent.preprocess_state(state)  # needs to reshape 
        action = np.argmax( agent.model.predict(state) )
        next_state, reward, done, _ = agent.env.step(action)
        state = next_state
        sum_reward += reward
        print( agent.model.predict(state) )
        if done: print(" done.  sum reward = ", sum_reward )
